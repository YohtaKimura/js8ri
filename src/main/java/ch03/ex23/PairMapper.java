...
public class PairMapper {
    ...
    public static <T, U> Pair<U> map(Pair<T> p, Function<T, U> func) {
		return new Pair<>(func.apply(p.getKey()), func.apply(p.getKey()));
	}

	public static void main(String[] args) {
		// 国名-国際電話番号のペアから国番号-国際電話番号のペアへ変換を行う
		Pair<String> p1 = new Pair<>("Japan", "81");
		Pair<Integer> p2 = PairMapper.map(p1, (str) -> {
			if ("Japan" == str) {
				return 392;
			} else {
